	...
	$(function () {
	  ...
	  function Slider(selector, settings) {
		...
		if (this.defaultSettings.multiSlide) {
		  ...
		  this.sliderArr[this.sliderArr.length - 1].classList.add(
			"js-ag-slide__prev"
		  );
		...
		this.slider.addEventListener("click", function (event) {
		  ...
		  if (
			target.closest(".js-ag-slide__prev") ||
			target.closest(".js-ag-slide_arrow__prev")
		  ) {
			_self.slidePrev();
		  ...
		  if (
			target.closest(".js-ag-slide__next") ||
			target.closest(".js-ag-slide_arrow__next")
		  ) {
			_self.slideNext();
	  ...
	  function SliderSwipe(selector, settings) {
		...
		if (
		  this.defaultSettings.progress &&
		  !this.inSel(".js-ag-progress-block")
		) {
		...
		function fnF() {
		  ...
		  if (_self.defaultSettings.progress) {
			var transformP =
			  ((_self.div.clientWidth - _self.divP.clientWidth) / 100) *
			  (startX / (rightEnd / 100));
	  ...
	  if (document.querySelector(".js-photo_container")) {
		var sliderSwipe = new SliderSwipe(".js-photo_container", {
		  progress: true
		});
		var sliderUnder = new Slider(".js-photo-block", {
		  multiSlide: false,
		  dots: true,
		  arrows: false,
		  infinity: false
